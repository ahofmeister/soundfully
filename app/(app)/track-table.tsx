              ...
              <DropdownMenuSubContent className="w-48">
                {playlists.map((playlist) => (
                  <DropdownMenuItem
                    ...
                    onClick={async (e) => {
                      e.stopPropagation();
                      await addToPlaylistAction(playlist.id, track.id);
                    }}
                  >
                    {playlist.name}
                  </DropdownMenuItem>
...
export function TrackTable({ playlist }: { playlist: Song[] }) {
  const [selectedTrackId, setSelectedTrackId] = useState<string | null>(null);

  let { registerPanelRef, setActivePanel, setPlaylist, activePanel } =
    usePlayback();
